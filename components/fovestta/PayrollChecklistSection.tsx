...
import { submitChecklistLead } from "@/lib/forms/actions";
...
const CHECKLIST_CONTENT = `FOVESTTA INDIA PAYROLL COMPLIANCE CHECKLIST
=============================================

1. PROVIDENT FUND (EPF)
------------------------
- Mandatory once your organisation has 20 or more employees.
- Employee contributes 12% of basic + DA; employer matches 12%
  (8.33% to EPS, capped at Rs 1,250/month, remainder to EPF).
- Employees earning up to Rs 15,000/month basic+DA must be
  enrolled; those above may opt in voluntarily.
- File ECR (Electronic Challan-cum-Return) and deposit
  contributions monthly via the EPFO portal.

2. EMPLOYEE STATE INSURANCE (ESI)
-----------------------------------
- Mandatory for establishments with 10 or more employees.
- Applies only to employees earning up to Rs 21,000/month
  (Rs 25,000/month for persons with disabilities).
- Employer contributes 3.25%, employee contributes 0.75%
  (4% total), rates unchanged since July 2019.

3. PROFESSIONAL TAX (PT)
--------------------------
- State-levied, not central. Applicability and slabs vary by
  state; check your state's Professional Tax Act.
- Capped at Rs 2,500 per employee per year under Article 276
  of the Constitution.

4. TAX DEDUCTED AT SOURCE (TDS)
----------------------------------
- Deduct TDS on salaries per the applicable income tax slabs.
- Deposit by the 7th of the following month (for March
  deductions, deadline is April 30).
- File Form 24Q quarterly (due 31 Jul, 31 Oct, 31 Jan, 31 May).
- Issue Form 16 to employees annually.

5. POSH ACT (PREVENTION OF SEXUAL HARASSMENT)
------------------------------------------------
- Mandatory Internal Committee (IC) once you have 10 or more
  employees (includes contract staff, interns, consultants).
- At least 4 members; a senior woman employee presides, one
  external member (NGO/legal background) required.

6. GRATUITY ACT
------------------
- Applies to establishments with 10 or more employees.
- Payable to employees who complete 5+ years of continuous
  service, on resignation, retirement, or termination.

7. PAYSLIPS & WAGE RECORDS
------------------------------
- Issue monthly payslips with a clear breakup of earnings and
  deductions (Payment of Wages Act).
- Maintain wage registers and attendance records for audit
  readiness.

8. MINIMUM WAGES
--------------------
- Minimum wage rates are set per state and per skill category
  (unskilled/semi-skilled/skilled) and revised periodically -
  check your state's latest notification.

This checklist is provided for general guidance and reflects
regulations understood to be current as of publication. It is
not legal advice - verify current rates and thresholds with a
qualified professional or the relevant government portal before
...
function downloadChecklist() {
  ...
  a.href = url;
  a.download = "fovestta-payroll-compliance-checklist.txt";
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 grid lg:grid-cols-[1.15fr_0.95fr] gap-10 lg:gap-16 items-center">
          ...
          >
            ...
            <div className="relative bg-white/80 backdrop-blur-xl rounded-[32px] p-6 lg:p-8 shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-white" style={{ transform: "translateZ(30px)" }}>
              ...
              <p className="text-[15px] lg:text-[16px] text-gray-500 font-medium mb-6">Instant download of the 2026 edition.</p>
              ...
              <form
                ...
                onSubmit={(e) => {
                  ...
                  downloadChecklist();
                  submitChecklistLead({ fullName, workEmail, companyName });
