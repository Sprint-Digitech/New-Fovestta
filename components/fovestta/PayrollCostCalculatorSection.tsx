...
export function PayrollCostCalculatorSection() {
  ...
  // ESI only applies to employees earning up to the statutory wage ceiling of ₹21,000/month.
  const ESI_WAGE_CEILING = 21000;
  const esiEligible = salary <= ESI_WAGE_CEILING;

  // Simple calculation logic for UI responsiveness
  const pfContribution = isPF ? (salary * 0.12) : 0;
  const esiContribution = (isESI && esiEligible) ? (salary * 0.0325) : 0;
          ...
          <div className="grid lg:grid-cols-[1fr_360px] gap-6 lg:gap-8 items-start">
            ...
            >
              ...
              <div className="space-y-8">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <motion.label
                    whileHover={{ scale: 1.02, translateZ: 10 }}
                    className={`flex items-center gap-3 p-4 rounded-2xl border cursor-pointer transition-all ${isESI && esiEligible ? 'bg-blue-50 border-[#3B82F6] shadow-sm' : 'bg-white border-gray-100'}`}
                  >
                ...
                {isESI && !esiEligible && (
                  <p className="text-[13px] lg:text-[14px] text-amber-700 bg-amber-50 border border-amber-100 rounded-xl px-4 py-3 -mt-2">
                    ESI doesn&apos;t apply here — it only covers employees earning up to the ₹{ESI_WAGE_CEILING.toLocaleString('en-IN')}/month statutory wage ceiling.
                  </p>
                )}
              </div>
            </motion.div>
            ...
            >
              ...
              <motion.button
                ...
                onClick={() => {
                  const lines = [
                    "FOVESTTA PAYROLL COST ESTIMATE",
                    "================================",
                    "",
                    `Team size: ${employees} employees`,
                    `Average monthly salary: ₹${salary.toLocaleString('en-IN')}`,
                    `Employer PF (12%): ${isPF ? "Included" : "Not included"}`,
                    `Employer ESI (3.25%): ${isESI && esiEligible ? "Included" : esiEligible ? "Not included" : `Not applicable (salary exceeds ₹${ESI_WAGE_CEILING.toLocaleString('en-IN')}/month ESI wage ceiling)`}`,
                    "",
                    "--------------------------------",
                    `Cost per employee: ₹${totalCostPerEmp.toLocaleString('en-IN')}`,
                    `Total PF contribution: ₹${(pfContribution * employees).toLocaleString('en-IN')}`,
                    `Total ESI contribution: ₹${(esiContribution * employees).toLocaleString('en-IN')}`,
                    `Total monthly cost: ₹${totalMonthlyCost.toLocaleString('en-IN')}`,
                    "--------------------------------",
                    "",
                    "This is an estimate for budgeting purposes only, based on",
                    "the inputs above and current statutory rates. It does not",
                    "account for wage-ceiling caps on PF, state-specific",
                    "professional tax, or other statutory deductions.",
                    "",
                    "Generated by Fovestta - fovestta.com",
                  ];
                  const blob = new Blob([lines.join("\n")], { type: "text/plain" });
                  const url = URL.createObjectURL(blob);
                  const a = document.createElement("a");
                  a.href = url;
                  a.download = "fovestta-payroll-cost-estimate.txt";
